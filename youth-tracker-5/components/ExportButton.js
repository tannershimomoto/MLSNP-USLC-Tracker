...
export default function ExportButton({ league, sortState }) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      // Pre-calculate total pages so headers are accurate
      const firstClubY = 44
      const contClubY = 34
      const clubsOnFirstPage = Math.floor((pageBottom - firstClubY) / rowH)
      const clubsOnContPage = Math.floor((pageBottom - contClubY) / rowH)
      const extraClubPages = clubs.length <= clubsOnFirstPage
        ? 0
        : Math.ceil((clubs.length - clubsOnFirstPage) / clubsOnContPage)
      const totalPages = 1 + 1 + extraClubPages
      ...
      function drawBirthYearLegend(y) {
        let lx = M
        doc.setFontSize(7)
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(90, 90, 90)
        doc.text('Birth year key:', lx, y + 2)
        lx += 22
        YEARS.forEach(yr => {
          ...
          doc.roundedRect(lx, y - 1.5, 16, 5, 1, 1, 'FD')
          doc.setTextColor(...c.text)
          doc.setFont('helvetica', 'bold')
          doc.text(String(yr), lx + 8, y + 2.2, { align: 'center' })
          lx += 19
        })
      }
      ...
      const tableRows = players.map((p, i) => [
        i + 1, p.name, p.team, p.position, p.birth_year, p.apps, p.mins, p.goals, p.assists
      ])
      ...
      // Section title + legend on first club page
      doc.setTextColor(...accentRGB)
      doc.setFontSize(9)
      doc.setFont('helvetica', 'bold')
      doc.text('Club minutes breakdown — total minutes given to 2008–2012 players, sorted by total', M, 29)
      doc.setDrawColor(...accentRGB)
      doc.setLineWidth(0.4)
      doc.line(M, 31, pageW - M, 31)
      drawBirthYearLegend(36)

      let cy = firstClubY

      for (let i = 0; i < clubs.length; i++) {
        // Check if we need a new page before drawing this club
        if (cy + rowH > pageBottom) {
          ...
          doc.setFontSize(9)
          doc.setFont('helvetica', 'bold')
          doc.text('Club minutes breakdown (continued)', M, 28)
          doc.setDrawColor(...accentRGB)
          doc.setLineWidth(0.4)
