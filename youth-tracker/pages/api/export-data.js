...
export default async function handler(req, res) {
  ...
  const [pRes, mRes] = await Promise.all([
    supabase.from('appearances').select(`
      ...
      players!inner(id, name, team, position, birth_year, league, agency),
      matches!inner(id, home_team, away_team, score, match_date, league)
    `).eq('players.league', league).eq('matches.league', league),
    supabase.from('matches').select('id, home_team, away_team, score, match_date').eq('league', league).order('match_date', { ascending: false })
  ])
  ...
  for (const row of pRes.data) {
    ...
    const key = p.id
    if (!playerMap[key]) {
      playerMap[key] = { id: p.id, name: p.name, team: p.team, position: p.position, birth_year: p.birth_year, agency: p.agency, apps: 0, mins: 0, goals: 0, assists: 0 }
    }
    playerMap[key].apps++
  ...
  const players = Object.values(playerMap).sort((a, b) => b.mins - a.mins)
  ...
  res.status(200).json({ players, clubs, matchCount: mRes.data?.length || 0 })
}
