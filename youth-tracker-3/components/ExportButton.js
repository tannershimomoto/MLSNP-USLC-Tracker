...
export default function ExportButton({ league, sortState }) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      // Stat summary boxes
      const totalMins = allPlayers.reduce((s, p) => s + p.mins, 0)
      const clubCount = new Set(allPlayers.map(p => p.team)).size
      const statBoxes = [
        { label: 'Players tracked', value: String(allPlayers.length) },
        { label: 'Clubs giving minutes', value: String(clubCount) },
        { label: 'Total minutes', value: totalMins.toLocaleString() },
      ...
      // Sort label
      const sortLabels = { mins: 'total minutes', name: 'player name', team: 'club', birth_year: 'birth year', apps: 'appearances', goals: 'goals', assists: 'assists', position: 'position' }
      const filterParts = []
      ...
      // Player table — landscape A4 gives plenty of width
      ...
      const labelColW = 72
      const numColW = 22
      const barColW = barAreaW - labelColW - numColW
      const rowH = 11
      let cy = 44
      ...
      clubs.forEach(club => {
        ...
        const displayName = club.team.length > maxNameChars ? club.team.slice(0, maxNameChars - 1) + '…' : club.team
        ...
        // Total minutes (right-aligned in label col)
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(...accentRGB)
        doc.setFontSize(8)
        doc.text(`${club.total.toLocaleString()}`, M + labelColW - 2, cy + 3.5, { align: 'right' })
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(120, 120, 120)
        doc.setFontSize(6.5)
        doc.text('min', M + labelColW - 2, cy + 8, { align: 'right' })
        ...
        // Year breakdown text next to bar
